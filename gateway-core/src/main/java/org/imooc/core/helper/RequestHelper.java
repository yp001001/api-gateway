...
public class RequestHelper {
    ...
    public static GatewayContext doContext(FullHttpRequest request, ChannelHandlerContext ctx) {
        ...
        GatewayContext gatewayContext = httpGatewayContext(request, ctx);

        return gatewayContext;
    }

    private static GatewayContext httpGatewayContext(FullHttpRequest request, ChannelHandlerContext ctx) {
        ...
        ConcurrentHashMap<String, ConfigServerData> configServerDataMap = instance.getConfigServerDataMap();
        ConfigServerData configServerData = configServerDataMap.get(gateWayRequest.getUniqueId());
        if (null == configServerData) {
            throw new RuntimeException("Unable to find corresponding service information");
        }
        ServiceDefinition serviceDefinition = DynamicConfigManager.getInstance().getServiceDefinition(configServerData.getDataId());
    ...
    private static GatewayRequest doRequest(FullHttpRequest fullHttpRequest, ChannelHandlerContext ctx) {
        ...
        String severPrefixPath = "/" + uri.split("/")[1];
        ...
        GatewayRequest gatewayRequest = new GatewayRequest(severPrefixPath,
                charset,
                clientIp,
                host,
                uri,
